      ...
      >标记为
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="star">
            <icon-svg icon-class="favourite" class="download-icon"/>星标邮件
          </el-dropdown-item>
...
export default {
  ...
  computed: {
    showMailTime() {
      return this.mail.receiveDate || this.mail.sendDate;
    }
  },
  methods: {
    initPage() {
    ...
    getDetail() {
      ...
      mailDetailAPI.fetchDetail(query).then(res => {
        ...
        detail.oldFileList = (detail.oldFileList || []).map(item => Object.assign({}, item, {
          url: resolveApiUrl(item.url)
        }));
        detail.oldAudioList = (detail.oldAudioList || []).map(item => Object.assign({}, item, {
          url: resolveApiUrl(item.url)
        }));
        this.mail = detail;
      });
    ...
    deleteMail() {
      this.$confirm('是否删除这封邮件', '确认', {
        ...
        .then(() => {
          mailDetailAPI.delMail(this.mail.id).subscribe({
            next: () => {
              this.$message({
                type: 'success',
                message: '删除成功',
